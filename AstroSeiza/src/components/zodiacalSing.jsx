...
export default function ZodicalSing() {
  ...
  const fetchZodiacalSign = async () => {
    try {
      setLoading(true);
      const response = await axios.post("http://localhost:3001/zodiaco", { birthdate });
      setZodiacSign(response.data.sign);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col w-auto justify-center mt-[50px]">
      <div className="grid zodiacalBG p-10 m-auto">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 items-center justify-center">
        ...
        {!loading && zodiacSign && <p className=" mt-10 text-white text-center">Tu signo del zodiaco: {zodiacSign}</p>}
        <div>
          <Lottie animationData={example} className="h-[250px]" />
        </div>
      </div>
    </div>
  );
